...
namespace yogi::visitor {
    ...
    std::any DictionaryVisitor::visitDictionaryDeclaration(Grammar::DictionaryDeclarationContext *ctx) {
        ...
        const auto visitDictionaryBody = visit(ctx->dictionaryBody());
        auto bodyNode = std::any_cast<std::vector<DictionaryPairNode> >(visitDictionaryBody);

        for (auto pairNode: bodyNode) {
            decl->pairs.push_back(pairNode);
        }

        decl->pairs.append_range(bodyNode);
        decl->size = decl->pairs.size();

        return std::shared_ptr<DictionaryDeclarationNode>(decl);
    }

    std::any DictionaryVisitor::visitDictionaryBody(Grammar::DictionaryBodyContext *ctx) {
        std::vector<DictionaryPairNode> pairs;
    ...
    std::any DictionaryVisitor::visitDictionaryAssignmentBody(Grammar::DictionaryAssignmentBodyContext *ctx) {
        ...
        const auto valAny = ctx->expression() ? visit(ctx->expression()) : keyAny;
