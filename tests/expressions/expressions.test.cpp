...
TEST_CASE("Binary expression evaluation", "[BINARY_EXPRESSION]") {
    ...
    std::string ctx = text;
    const auto ast = cromio::Cromio::testAST(ctx);
    const auto& expression = std::any_cast<cromio::visitor::nodes::BinaryExpressionNode>(ast.body[0].children.at(0));

    // Top-level checks
    REQUIRE(expression.kind == cromio::visitor::nodes::Kind::BINARY_EXPRESSION);
    REQUIRE(expression.op == expected_op);
    REQUIRE(expression.resultType == expected_type);

    // Left operand
    const auto [lType, lValue, lNode] = cromio::utils::Helpers::resolveItem(expression.left);
    INFO("Left operand: " << lValue);
    REQUIRE(!lValue.empty());

    // Right operand
    const auto [rType, rValue, rNode] = cromio::utils::Helpers::resolveItem(expression.right);
